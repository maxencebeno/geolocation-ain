$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                for (var i = 0; i < data.length; i++) {
                    division.append("<option value='" + data[i].id + "'>" + data[i].code + " " + data[i].libelle + "</option>");
                }
    ...
    function getGroupe(sectionId, divisionId, groupe) {
        $.ajax({
            ...
            success: function (data) {
                for (var i = 0; i < data.length; i++) {
                    groupe.append("<option value='" + data[i].id + "'>" + data[i].code + " " + data[i].libelle + "</option>");
                }
    ...
    function getClasse(sectionId, divisionId, groupeId, classe) {
        $.ajax({
            ...
            success: function (data) {
                for (var i = 0; i < data.length; i++) {
                    classe.append("<option value='" + data[i].id + "'>" + data[i].code + " " + data[i].libelle + "</option>");
                }
